...
public record ImportSnapshot(int RowCount, List<string> Headers, List<Dictionary<string, string>> PreviewRows, List<ColumnProfile> Profile, List<Anomaly> Anomalies, List<TransformRule> SuggestedRules, BenchmarkMetrics Benchmark, string SuggestionProvider);
public record CsvAnalysisResult(List<string> Headers, int RowCount, List<Dictionary<string, string>> PreviewRows, List<ColumnProfile> Profile, List<Anomaly> Anomalies, List<TransformRule> SuggestedRules, BenchmarkMetrics Benchmark, string SuggestionProvider);
...
public record BenchmarkMetrics(long ExecutionMs, double RowsPerSecond, long PeakMemoryBytes, LatencySummary Latency);
public record LatencySummary(double P50Ms, double P95Ms, double MaxMs, int SampleCount);
